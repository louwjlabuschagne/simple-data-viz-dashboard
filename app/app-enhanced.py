...
dropdown = dcc.Dropdown(
    options=[{'label': c, 'value': c} for c in characters],
    value=characters[0],
    multi=True,
...
@app.callback(
    ...
    Input(component_id='selected-character', component_property='value')
)
def update_output_div(chars):
    if type(chars) == str:
        chars = [chars]

    df = df_mentions[df_mentions['character'].isin(chars)]
    ...
    fig = go.Figure(data=[go.Sankey(
        ...
        link=dict(
            source=df['speaker'].values,
            target=df['mentioner'].values,
